...
response = client.chat.completions.create(      # Send the request to the AI
    model="deepseek-v4-flash"   ,        # Which model to use
    messages=[              
        {"role": "system", "content": "You are a pirate. Answer everything in pirate speak."},                     # The conversation content
        {"role": "user", "content": "What is Python in 2 sentences?"}
    ],
    temperature=1.0,    # <-- the new parameter
)
